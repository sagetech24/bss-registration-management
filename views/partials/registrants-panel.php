<?php
/**
 * Shared registrants table panel (Alpine-driven).
 * Expects $registrants_config array with apiUrl, paymentDetailsUrl, profileUrl, eventId, eventIsFree, initialPackageFilter.
 */
if (!isset($registrants_config) || !is_array($registrants_config)) {
    return;
}
?>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('rmEventRegistrants', () => ({
        ...
        openGuestsModal(row) {
            ...
            this.guestsModalPrimary = 'Primary Registrant: ' + (row.full_name || '') + (row.order_number ? ' · ' + row.order_number : '');
                            ...
                            <div class="rounded-lg border border-slate-200 bg-slate-50/80 p-4">
                                <div class="flex flex-wrap items-start justify-between gap-2">
                                    <div>
                                        ...
                                        <p class="mt-0.5 text-sm font-mono text-slate-500" x-text="guest.order_number || 'N/A'"></p>
                                    </div>
                                    <div class="flex flex-wrap items-center gap-1.5">
                                        <span
